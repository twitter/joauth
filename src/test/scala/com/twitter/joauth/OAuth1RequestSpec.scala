...
class OAuth1RequestSpec extends Specification {
  "OAuth1Request.verify" should {
    "throw on null scheme" in {
      OAuth1Request.verify(null, "2", 3, "4", "5", new OAuthParams) must throwA(new MalformedRequest("no value for scheme"))
    }
    "throw on null host" in {
      OAuth1Request.verify("1", null, 3, "4", "5", new OAuthParams) must throwA(new MalformedRequest("no value for host"))
    }
    "throw on null port" in {
      OAuth1Request.verify("1", "2", -1, "4", "5", new OAuthParams) must throwA(new MalformedRequest("no value for port"))
    }
    "throw on null verb" in {
      OAuth1Request.verify("1", "2", 3, null, "5", new OAuthParams) must throwA(new MalformedRequest("no value for verb"))
    }
    "throw on null path" in {
      OAuth1Request.verify("1", "2", 3, "4", null, new OAuthParams) must throwA(new MalformedRequest("no value for path"))
    }
    "throw on null signature method" in {
      OAuth1Request.verify("1", "2", 3, "4", "5", new OAuthParams) must throwA(new MalformedRequest("unsupported signature method: null"))
    }
